...
class Like(Model):
    bookmark = ForeignKey(Bookmark, on_delete=models.CASCADE, related_name='likes', null=True)
    comment = ForeignKey(Comment, on_delete=models.CASCADE, related_name='likes', null=True)

    def __str__(self):
        return '{} like'.format(self.bookmark.link if self.bookmark else self.comment.link)
